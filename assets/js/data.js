...
// ============================================================
const TRIP_COLORS = [
  '#B8D4E8','#A8E2DC','#B0E8C8','#C8EAA8','#F0EAA8',
  '#FFE0A8','#FFCCB0','#FFB8B8','#FFB8D0','#F8C0DC',
  '#F0C0E8','#E0C0F0','#CEB8F0','#C0C8F0','#B8C8E8',
  '#B0D8CC','#D8E8B0','#E8D8B0','#D8C0E8','#C0E0E8'
];
...
const INITIAL_DATA = {
  ...
  atividades: [
    { id:'a1', nome:'Batimetria',     categoria:'Segurança',  icone:'📡', status:'Ativa', ordem:1 },
    { id:'a2', nome:'Inspeção',       categoria:'Segurança',  icone:'🔍', status:'Ativa', ordem:2 },
    { id:'a3', nome:'Coleta de Dados',categoria:'Segurança',  icone:'📊', status:'Ativa', ordem:3 },
    { id:'a4', nome:'Cadastro',       categoria:'Emergência', icone:'📋', status:'Ativa', ordem:4 },
    { id:'a5', nome:'Simulado',       categoria:'Emergência', icone:'🎯', status:'Ativa', ordem:5 },
    { id:'a6', nome:'Workshop',       categoria:'Emergência', icone:'📚', status:'Ativa', ordem:6 },
    { id:'a7', nome:'Placas',         categoria:'Emergência', icone:'🪧', status:'Ativa', ordem:7 },
    { id:'a8', nome:'Reunião',        categoria:'Emergência', icone:'🤝', status:'Ativa', ordem:8 },
    { id:'a9', nome:'Evento',         categoria:'Emergência', icone:'🎪', status:'Ativa', ordem:9 },
  ],
  viagens: [
    {
      id:'V01',
      ...
      paradas:[
        {
          ...
          atividades:['a4'],
          colaboradores:[
            { colaboradorId:'c17', dataInicio:'2026-06-27', dataFim:'2026-07-05', confirmado:false },
            { colaboradorId:'c18', dataInicio:'2026-06-27', dataFim:'2026-07-05', confirmado:false },
            { colaboradorId:'c19', dataInicio:'2026-06-27', dataFim:'2026-07-05', confirmado:false },
            { colaboradorId:'c1',  dataInicio:'2026-06-27', dataFim:'2026-06-30', confirmado:false },
          ]
        }
      ]
    },
      ...
      paradas:[
        {
          ...
          atividades:['a6','a5'],
          colaboradores:[
            { colaboradorId:'c9',  dataInicio:'2026-07-05', dataFim:'2026-07-08', confirmado:false },
            { colaboradorId:'c20', dataInicio:'2026-07-05', dataFim:'2026-07-08', confirmado:false },
            { colaboradorId:'c18', dataInicio:'2026-07-05', dataFim:'2026-07-08', confirmado:false },
            { colaboradorId:'c19', dataInicio:'2026-07-05', dataFim:'2026-07-08', confirmado:false },
          ]
        }
      ]
    }
...
class Database {
  ...
  importJSON(json) {
    try {
      ...
      if (!parsed.viagens || !parsed.colaboradores) throw new Error('Estrutura inválida');
      this._data = parsed;
      this.save();
      return { ok:true };
    } catch(e) {
  ...
  // ---- helpers ----
  getViagem(id)        { return this.data.viagens.find(v => v.id === id); }
  getColaborador(id)   { return this.data.colaboradores.find(c => c.id === id); }
  getEmpreendedor(id)  { return this.data.empreendedores.find(e => e.id === id); }
  getAtividade(id)     { return this.data.atividades.find(a => a.id === id); }
  ...
  getViagemColor(viagemId) {
    ...
    if (!v) return '#E0E0E0';
    const idx = (v.colorIndex !== undefined ? v.colorIndex : 0) % TRIP_COLORS.length;
  ...
  nextViagemId() {
    const ids = this.data.viagens.map(v => parseInt(v.id.replace('V',''),10)).filter(n => !isNaN(n));
